describe('Response validation for GET user request', () => {
  ...
  it('validates response status code  for getting a valid user request', () => {
    cy.request({method:'GET', url:'/api/v1/Users/5',  failOnStatusCode: false}).as('GetUser5')
    cy.get('@GetUser5').its('status').should('equals', 200)
  })
